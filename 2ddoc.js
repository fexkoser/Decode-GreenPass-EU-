...
const CERTIFICATE_AUTHORITIES = new Map([
 ...
 function getCertificateAuthority(certificateAuthorityId) {
...
const PUBLIC_KEYS = new Map([
 ...
 function getPublicKey(publicKeyId) {
...
// 7.14. Identifiants de données relatives aux résultats des tests virologiques - F5 (Page 81)
 function getAnalysisResult(analysisResult) {
...
function parse_2ddoc_date(date_str) {
    /**
...
function extract_data(code, fields, o) {
    const document_data = Object.fromEntries(fields.map((f) => {
        if (!(f.name in o))
            throw new Error(`Missing data for field ${f.name}`);
...
const SIGNATURE_REGEX = '\\x1F{1}' + // This character is separating the message from its signature.
    '(?<signature>[A-Z\\d\\=]+)'; // 14 - This is the message signature.
const TOTAL_REGEX = new RegExp(`^(?<data>${HEADER_REGEX}(?:${VACCINE_REGEX}|${TEST_REGEX}))${SIGNATURE_REGEX}$`);
...
module.exports = { 
  parse: async function parse(doc) {
    var _a;
    const groups = (_a = doc.match(TOTAL_REGEX)) === null || _a === void 0 ? void 0 : _a.groups;
    if (!groups)
        throw new Error('Format de certificat invalide');
